 ...
 */
public class AdditiveEmbeddingsStackedRNN extends RNN {
  ...
  public AdditiveEmbeddingsStackedRNN(float learningRate, int seqLength, int hiddenLayerSize, int epochs, String text) {
    this(learningRate, seqLength, hiddenLayerSize, epochs, text, 1, true, false);
  }

  public AdditiveEmbeddingsStackedRNN(float learningRate, int seqLength, int hiddenLayerSize, int epochs, String text, int batch, boolean useChars, boolean rmsProp) {
  ...
  public void learn() {
    ...
    SentenceIterator iter = new CollectionSentenceIterator(this.data);
    ...
    labels = paragraphVectors.getLabelAwareIterator().getLabelsSource().getLabels();
  ...
  private double lossFun(INDArray inputs, INDArray targets, INDArray dWxh, INDArray dWhh, INDArray dWxh2, INDArray dWhh2, INDArray dWh2y,
    ...
    List<org.nd4j.linalg.primitives.Pair<String, Double>> scores = getScores(inputs);
    if (scores.size() > 0) {
      vec0 = paragraphVectors.getLookupTable().vector(scores.get(1).getFirst()).transpose();
      vec1 = paragraphVectors.getLookupTable().vector(scores.get(0).getFirst()).transpose();
    }


//    String string = getSampleString(inputs);
//    Collection<String> strings = paragraphVectors.nearestLabels(string, 2);
//    int i = 0;
//    for (String s : strings) {
//      if (i == 0) {
//        vec0 = paragraphVectors.getLookupTable().vector(s);
//      } else {
//        vec1 = paragraphVectors.getLookupTable().vector(s);
//      }
//      i++;
//    }
    ...
    for (int t = 0; t < seqLength; t++) {
      ...
      hPrev = Transforms.tanh((wxh.mmul(xst.transpose()).add(whh.mmul(hPrev)).add(bh)).add(vec0)); // hidden state
      ...
      hPrev2 = Transforms.tanh((wxh2.mmul(hPrev).add(whh2.mmul(hPrev2)).add(bh2)).add(vec1)); // hidden state 2
  ...
  public String sample(int seedIx) {
    ...
    INDArray vec0 = null;
    INDArray vec1 = null;
    List<org.nd4j.linalg.primitives.Pair<String, Double>> scores = getScores(ixes);
    if (scores.size() > 0) {
      vec0 = paragraphVectors.getLookupTable().vector(scores.get(1).getFirst()).transpose();
      vec1 = paragraphVectors.getLookupTable().vector(scores.get(0).getFirst()).transpose();
    }
    ...
    for (int t = 0; t < sampleSize; t++) {
      h = Transforms.tanh((wxh.mmul(x)).add(whh.mmul(h)).add(bh).add(vec0));
      h2 = Transforms.tanh((wxh2.mmul(h)).add(whh2.mmul(h2)).add(bh2).add(vec1));
  ...
  private List<org.nd4j.linalg.primitives.Pair<String, Double>> getScores(INDArray vector) {
    List<org.nd4j.linalg.primitives.Pair<String, Double>> result = new ArrayList<>(labels.size());

    for (String label : labels) {
      INDArray vecLabel = paragraphVectors.getLookupTable().vector(label);
      if (vecLabel == null) {
        throw new IllegalStateException("Label '" + label + "' has no known vector!");
      }

      double sim = Transforms.cosineSim(vector, vecLabel);
      result.add(new org.nd4j.linalg.primitives.Pair<>(label, sim));
    }
    result.sort((o1, o2) -> {
      int i;
